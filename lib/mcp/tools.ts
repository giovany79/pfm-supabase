...
const validType = (v: unknown) =>
  v === 'income' || v === 'expensive' || v === 'saving';
const sqlLike = (value: unknown) =>
  typeof value === 'string' &&
  /\b(select|insert|update|delete|drop|alter|union)\b|--|;|\/\*/i.test(value);
export function assertReadInput(name: string, input: Record<string, unknown>) {
  const allowed: Record<string, string[]> = {
    query_transactions: ['date_from', 'date_to', 'category', 'type', 'limit'],
    query_snapshots: ['as_of_date', 'kind', 'category', 'institution'],
    aggregate_transactions: ['group_by', 'date_from', 'date_to', 'type'],
  };
  if (
    !allowed[name] ||
  ...
  if (input.type !== undefined && !validType(input.type))
...
function assertChange(input: Record<string, unknown>): ProposedChange {
  ...
  if (operation !== 'create' && operation !== 'edit' && operation !== 'delete')
    throw new Error('operation must be create, edit, or delete');
  ...
  if (
    ...
    throw new Error('missing target_transaction_id');
  if (
    result.type_income_expense !== undefined &&
    !validType(result.type_income_expense)
  )
    throw new Error('invalid type_income_expense');
  if (
...
function assertTransactionDraft(
  input: unknown,
  index: number,
): TransactionDraft {
  ...
  if (Object.keys(draft).some((key) => !allowed.includes(key)))
    throw new Error(`invalid transaction field at index ${index}`);
  ...
  if (!validType(draft.type_income_expense))
    throw new Error(`invalid type_income_expense at index ${index}`);
  if (
...
function assertBatchChange(
  input: Record<string, unknown>,
): ProposedBatchChange {
  if (Object.keys(input).some((key) => key !== 'transactions'))
    throw new Error('invalid batch input');
  if (!Array.isArray(input.transactions))
    throw new Error('transactions must be an array');
  if (input.transactions.length < 2 || input.transactions.length > 20)
    throw new Error('batch must contain between 2 and 20 transactions');
  return {
    operation: 'batch_create',
    transactions: input.transactions.map(assertTransactionDraft),
  };
}
export async function executeReadTool(
  ...
  if (name === 'query_transactions')
    result = await queryTransactions(client, input as any);
  else if (name === 'query_snapshots')
    result = await querySnapshots(client, input as any);
  else if (name === 'aggregate_transactions') {
    if (input.group_by !== 'category' && input.group_by !== 'month')
      throw new Error('group_by must be category or month');
    result = await aggregateTransactions(client, input as any);
  } else throw new Error('Unknown read tool');
  await logQuery(client, channel, name, input, result.row_count);
  return result;
}
export async function proposeTransactionChange(
  client: SupabaseClient,
  input: Record<string, unknown>,
) {
  ...
  const { data, error } = await client
    .from('pending_transaction_changes')
    .insert({
      owner_id: ownerId,
      operation: change.operation,
      target_transaction_id: change.target_transaction_id ?? null,
  ...
  return {
    ...
    summary: `${change.operation} transaction${change.target_transaction_id ? ` ${change.target_transaction_id}` : ''}`,
  };
}

export async function proposeTransactionBatch(
  client: SupabaseClient,
  input: Record<string, unknown>,
) {
  const change = assertBatchChange(input);
  const ownerId = await currentOwnerId(client);
  const expires_at = new Date(Date.now() + 5 * 60_000).toISOString();
  const total = change.transactions.reduce(
  ...
  return {
    ...
    expires_at: data.expires_at,
    summary,
    transaction_count: change.transactions.length,
    total_amount: total,
    transactions: change.transactions,
  };
}

export async function proposeSnapshotChange(
  client: SupabaseClient,
  input: Record<string, unknown>,
) {
...
export async function confirmSnapshotChange(
  ...
  if (new Date(pending.expires_at) < new Date()) {
    await client
      ...
      .delete()
      .eq('id', pending.id);
    await logSnapshotMutation(
      client,
      channel,
      change.operation,
      change.target_item_id ?? null,
      'failure',
    );
...
export async function confirmTransactionChange(
  client: SupabaseClient,
  channel: 'mcp' | 'action',
  pending_change_id: string,
) {
  ...
  if (error) throw error;
  if (!pending) return { outcome: 'failure', reason: 'not_found' };
  const change = pending.proposed_fields as
    ProposedChange | ProposedBatchChange;
  if ('entity' in change && change.entity === 'snapshot')
    return { outcome: 'failure', reason: 'not_found' };
  const auditOperation =
    change.operation === 'batch_create' ? 'create' : change.operation;
  const targetId =
    change.operation === 'batch_create'
      ? null
      : (change.target_transaction_id ?? null);
  if (new Date(pending.expires_at) < new Date()) {
    await client
      .from('pending_transaction_changes')
      .delete()
      .eq('id', pending.id);
    await logMutation(client, channel, auditOperation, targetId, 'failure');
    return { outcome: 'failure', reason: 'expired' };
  }
  try {
    if (change.operation === 'batch_create') {
      const affected = await createTransactionsBatch(
        client,
        change.transactions,
      );
      await client
      ...
      return {
        outcome: 'success',
        operation: 'batch_create',
        transaction_ids: affected.map(
          (transaction) => transaction.transaction_id,
        ),
        applied_count: affected.length,
      };
    }
    const affected = await applyMutation(client, change);
    await client
      .from('pending_transaction_changes')
      .delete()
      .eq('id', pending.id);
    ...
    return {
      outcome: 'success',
      operation: change.operation,
      transaction_id: affected.transaction_id,
      applied_fields: change.operation === 'delete' ? {} : change,
    };
  } catch (cause) {
    await client
      .from('pending_transaction_changes')
