...
- [X] T041 [US3] Implement the MCP Streamable HTTP server (`initialize`, `tools/list`, `tools/call`) per contracts/mcp-server.md in `app/api/mcp/route.ts` (depends on T039) — this endpoint serves the **optional** Claude.ai surface
...
- [X] T045 [US3] Implement the OpenAPI schema endpoint (`GET /api/actions/openapi.json`, all documented operations) per contracts/gpt-actions.md in `app/api/actions/openapi.json/route.ts` (depends on T042, T043, T044)
...
- [X] T049 [P] [US3] E2e smoke test: a direct `tools/call` request to `/api/mcp` and a direct request to each read `/api/actions/*` endpoint each return a grounded, logged result, and both reject a missing/invalid bearer token with `401`, in `tests/e2e/qa-endpoints.spec.ts`
...
## Phase 8: MCP agent clients

**Purpose**: Make the deployed MCP endpoint directly usable from Codex and Claude agents,
while preserving bearer authentication, RLS, grounding, and confirmed-only mutations.
